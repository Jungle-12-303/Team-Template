...
```
[X] thread_block ()           /* 잠들면 안 됨 */
[X] malloc / free             /* lock 획득 시도 */
[X] lock_acquire              /* lock 시도 */
[X] sema_down                 /* 블록 시도 */
[X] printf 등 큰 작업          /* 너무 오래 걸림 */
```

### 인터럽트 핸들러에서 허용

```
[O] thread_unblock           /* 깨우기는 OK */
[O] sema_up                  /* 시그널은 OK */
[O] list 조작                /* 빠른 조작 */
[O] 짧은 메모리 접근
```
...
[!] 자동 포매터는 PintOS 의 모든 관행을 완벽 재현 못함 (특히 인라인 주석 정렬). **자동화 결과를 절대 신뢰하지 말고**, 수동 검토 필수.
